...
impl Db{
	...
	pub fn upload_map(&mut self, player_name: &str, map_name: &str, map_data: Vec<u8>){
		self.conn.exec_batch(
			"INSERT INTO maps (owner, map_name, map_data) VALUES (?, ?, ?);",
			vec![
				Params::Positional(vec![mysql::Value::from(player_name), mysql::Value::from(map_name), mysql::Value::from(map_data)],),
			]).unwrap();
	}
	pub fn set_map_finished(&mut self, map_name: &str, finished: bool){
		self.conn.exec_batch(
			"UPDATE maps SET finished = ? WHERE map_name = ?;",
			vec![
	...
	pub fn get_finished_maps(&mut self) -> Vec<String>{
		let mut results = self.conn.exec_iter(
			"SELECT map_name FROM maps WHERE finished = 1;",
				Params::Empty,
			).unwrap();
		let mut maps = vec![];
	...
	pub fn upload_result(&mut self, player_name: &str, map_name: &str, time: f32){
		...
		self.conn.exec_batch(
			...
			vec![
				Params::Positional(vec![mysql::Value::from(player_name), mysql::Value::from(map_name), mysql::Value::from(time)],),
			]).unwrap();
	}
